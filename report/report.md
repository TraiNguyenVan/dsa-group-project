...
**Hashing & collisions** — maps key→bucket directly: O(1) expected lookup/insert/delete regardless of n, no scan/halving/sorted order. Binary needs O(log n) lookup but O(n) insert; linear needs no order but O(n) lookup — neither scales for a phonebook growing by thousands/day. Built on bucket array + per-bucket chain: 64-bit poly hash `hash*31+(c-'0')` wrap, `% numBuckets` (`hashtable.cpp:HashForSize`; start 101→nextPrime, rehash load>0.75→nextPrime(2×), `isPrime` 6k±1); collisions share a chain searched linearly. Foundation of dicts/caches/symbol tables. Wrong for range queries ("phones starting 09" — needs sorted/BST), hard worst-case guarantees (pathological collisions → O(n) chain vs BST O(log n)), or tight memory (low load wastes buckets).
