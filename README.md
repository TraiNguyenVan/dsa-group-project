...
└── benchmark/                  # generated by `make run-benchmark` / `run-memory`
    ├── plot.py                 # 5 modes + --line/--mem
    ├── mem_profile.py          # memory profiling driver (make run-memory)
    ├── results.csv
    ├── plot.png
    ├── plot-*.png              # per-algo, per-language, runtime-vs-n, memory charts
    ├── gallery.html            # chart gallery
    ├── package.json            # gallery deps (no npm install needed)
    ├── phonebook-go            # prebuilt Go binary (benchmark artifact)
    ├── massif-*.txt            # Massif ms_print output
    ├── rss-*.txt               # peak-RSS samples
    └── mem/                    # profiler evidence (pprof/.jfr/.heapprofile/massif)
```
