// largest_element.hpp — find the largest value in an int array
// Author:  @N25DECE074-truong-quang-tuan
// Source:  n/a
// AI use:  n/a
#ifndef LARGEST_ELEMENT_HPP
#define LARGEST_ELEMENT_HPP
...
inline int largest(int arr[], int n) {
...
// inline: definition lives in this header, so more than one .cpp may include it
inline int findMax(int arr[], int i, int n) {
...
// inline: definition lives in this header, so more than one .cpp may include it
inline int largestRescusion(int arr[], int n) {
    // Gọi findMax bắt đầu từ vị trí đầu tiên 0.
    return findMax(arr, 0, n);
}

#endif  // LARGEST_ELEMENT_HPP
